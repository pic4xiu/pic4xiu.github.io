...
这时候我很好奇之前函数为什么定义了那么多字符串，回去看看之前的`puts(dword_40AD8C)`就蒙了，这串数什么时候变的？向上找找到了`sub_401500(0);`这函数。我们分析一下
...
跳转之前改变了**dword_40ADA8**这个值我们通过它跳到了`sub_40157F`中
...
输入字符串与**u1nnf2lg**和2的异或进行比对，最后两个字符串拼接即得flag:)
