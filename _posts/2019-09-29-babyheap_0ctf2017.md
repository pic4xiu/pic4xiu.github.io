...
算是第一次正经研究堆的 ctf 题目,认真写一下,之前国赛的 wp 真是 orz ,虽然强,但是感觉和我目前技术完全脱节了,没长多少知识点,这次见识了,咱也不知道是不是每个堆题思路都这么骚,如果以栈题是一个一步一步脚踏实地的 getshell 的话,堆题就是一个天马行空的欺骗艺术外加扎实的技术
...
分析完总结一下,本题最难想的就是直接 malloc 到一个接近 malloc_hook 附近的位置,但是大佬直接找到 7f ,不过其实挺讨巧的,毕竟是 64 位,一个 7f 真的挺好找的,怎么说呀,不知道这是不是一个非预期解,再找找吧
