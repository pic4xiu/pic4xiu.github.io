...
首先我们要知道程序调用 exit 会调用`_IO_2_1_stdout_`的`sebuf`函数,所以把程序分成 2 次任意写,第一次完成把`_IO_2_1_stdout_`的`vtable`改写为`fake_vtable`,其中 fake_vtable 的目的是完成改写 `_IO_2_1_stdout_+88` 的 sebuf
