...
然后我就开始研究 classes.dex 源码了，先把 dex 转成 jar ，然后用jd-gui一顿 xjb 分析，大致看了一下有一个获取路径的类
...
用到了 lua ~~~，程序对 lua 进行加载和解密，进入`com\androlua\LuaActivity.class`,一个`public Object loadLib(String paramString)`加载 so 文件，之后就开始了 so 文件的排查解密函数

我目前的思路就是通过排查 so 进而逆向出 main.lua 了，如果 main.lua 没什么问题就是 java 写的了

通过几个关键函数`luaL_loadstring`,我们最终定位到了`j_luaL_loadbufferx`
...
我们发现逻辑其实十分清晰，只有一行关键加密，我们把文件加载进来，然后顺着整个脚本就能把 lua 解密成字节码形式了，卧槽，看见大佬直接调出来了，真是orz
...
可以看到 lua 只是找到了 0.mp3 ，然后甚至了一波路径之后疯狂循环音量控制，然后就没有然后了
