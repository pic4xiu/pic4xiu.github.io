...
这个 unlink 利用起来十分巧妙,是把堆的指针地址减去2或3构成这样(本来指针在0x602070)

```
pwndbg> x/32gx 0x0000000000602058
0x602058:	0x0000000000000000	0x00007ffff7dd2540
0x602068 <completed.7594>:	0x0000000000000000	0x0000000000603010 <- bk(0x602070)
```

```
pwndbg> x/32gx 0x0000000000602060
0x602060 <stderr@@GLIBC_2.2.5>:	0x00007ffff7dd2540	0x0000000000000000
0x602070 <chunk0_ptr>:	0x0000000000603010	0x0000000000000000 <- fd(0x602070)
...
构造完成,之后再伪装一下 chunk1 ,把 pre_size 和 size 都改了,让系统以为 chunk0 已经被释放了,之后直接释放 chunk1 就会合并,释放完后我们看一下
...
```
我们发现,他把 bk 改成了我们之前在 chunk0 写的,很完美,之后修改`0x602070`改成我们想的地址,之后用这个地址就能搞事情了
