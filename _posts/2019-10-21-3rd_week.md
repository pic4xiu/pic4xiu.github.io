...
这是我做过最难的 rop 了,必须泄漏三个( canary stack libc),太恶心了,不过思路还是很清楚的,我试试优化一下,最后我感觉师傅写错了,但是 exp 却能用,又迷了~~~~ 学到了.这个栈迁移是迁移到 buf 内部.下面说一下我的方法, one_gadget 一步 getshell ,只是简化了第三步,就不用师父的模板了,实在使不顺手,不过师傅的 gdb 函数是真的牛逼~~
